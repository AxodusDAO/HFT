...
from hummingbot.strategy.whiterabbit.data_types import PriceSize, Proposal
from hummingbot.strategy.whiterabbit.whiterabbit_order_tracker import (
    WhiteRabbitOrderTracker,
)
...
class WhiteRabbitStrategy(StrategyPyBase):
    ...
    def init_params(self,
        ...
        self._safe_stop_rate = safe_stop_rate
    ...
    def stop_loss_proposal(self, mode: PositionMode, active_positions: List) -> Proposal:
        ...
        for position in active_positions:
            if (top_bid < position.entry_price and position.amount > 0) or (
                ...
                price = market.quantize_order_price(self.trading_pair, stop_price)
                size = market.quantize_order_amount(self.trading_pair, abs(position.amount))
        ...
        for position in active_positions:
            ...
            if (not existent_stop_loss_orders
                ...
                if (top_ask <= stop_loss_price and position.amount > 0):
                    ...
                    size = market.quantize_order_amount(self.trading_pair, abs(position.amount))
                    if size > 0 and price > 0:
                        self.logger().info("SAFE STOP: Creating sell order to close long position.")
                        sells.append(PriceSize(price, size))
        ...
        if len(proposal.buys) > 0:
            ...
            if self._logging_options & self.OPTION_LOG_CREATE_ORDER:
                price_quote_str = [f"{buy.size.normalize()} {self.base_asset}, "
                                   ...
                                   for buy in proposal.buys]
                self.logger().info(
                    f"({self.trading_pair}) BUY: Creating {len(proposal.buys)} {self._safe_stop_order_type.name} bid orders "
                    f"at (Size, Price): {price_quote_str} to {position_action.name} position."
                )
            for buy in proposal.buys:
                bid_order_id = self.buy_with_specific_market(
        ...
        if len(proposal.sells) > 0:
            ...
            if self._logging_options & self.OPTION_LOG_CREATE_ORDER:
                price_quote_str = [f"{sell.size.normalize()} {self.base_asset}, "
                                   ...
                                   for sell in proposal.sells]
                self.logger().info(
                    f"({self.trading_pair}) SELL: Creating {len(proposal.sells)}  {self._safe_stop_order_type.name} ask "
                    f"orders at (Size, Price): {price_quote_str} to {position_action.name} position."
                )
            for sell in proposal.sells:
                ask_order_id = self.sell_with_specific_market(
