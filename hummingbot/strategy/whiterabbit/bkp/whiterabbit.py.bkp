                    ...
                    if (top_ask <= stop_loss_price and position.amount > 0):
                        ...
                        if size > 0 and price > 0:
                            self.logger().info(
                                ...
                                f"at (Size, Price): {price_quote_str} to {position_action.name} long position.")                            
                            sells.append(PriceSize(price, size))
                        ...
                        if size > 0 and price > 0:
                            self.logger().info(
                                f"({self.trading_pair}) Creating {len(proposal.buys)} stop loss buy order"
                                f"at (Size, Price): {price_quote_str} to {position_action.name} short position.")
                            buys.append(PriceSize(price, size))
