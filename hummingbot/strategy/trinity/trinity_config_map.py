...
def maker_trading_pair_prompt():
    derivative = trinity_config_map.get("derivative").value
...
def order_amount_prompt() -> str:
    trading_pair = trinity_config_map["market"].value
...
def on_validate_price_source(value: str):
    if value != "external_market":
        trinity_config_map["price_source_derivative"].value = None
        trinity_config_map["price_source_market"].value = None
    if value != "custom_api":
        trinity_config_map["price_source_custom_api"].value = None
    else:
        trinity_config_map["price_type"].value = "custom"


def validate_price_type(value: str) -> Optional[str]:
    error = None
    price_source = trinity_config_map.get("price_source").value
...
def price_source_market_prompt() -> str:
    external_market = trinity_config_map.get("price_source_derivative").value
    return f'Enter the token trading pair on {external_market} >>> '


def validate_price_source_derivative(value: str) -> Optional[str]:
    if value == trinity_config_map.get("derivative").value:
...
def validate_price_source_market(value: str) -> Optional[str]:
    market = trinity_config_map.get("price_source_derivative").value
...
def on_validated_ma_cross_enabled(value: str):
    if value.lower() == "enabled":
        trinity_config_map["fast_ma"].value = None
        trinity_config_map["slow_ma"].value = None
    else:
        if value.upper() == "SMA":
            trinity_config_map["fast_ma"].prompt = "Enter the FAST SMA time period >>> "
            trinity_config_map["slow_ma"].prompt = "Enter the SLOW SMA time period >>> "
        elif value.upper() == "EMA":
            trinity_config_map["fast_ma"].prompt = "Enter the FAST EMA time period >>> "
            trinity_config_map["slow_ma"].prompt = "Enter the SLOW EMA time period >>> "
   ...
   # Golden / Death Cross strategy
    ...
    "ma_type":
        ConfigVar(key="ma_type",
                  ...
                  required_if=lambda: trinity_config_map.get("ma_cross_enabled").value,
                  prompt_on_new=True,
                  default="SMA"),
    "fast_ma":
   ...
   "moving_price_band_enabled":
    ...
    "price_band_refresh_time":
        ConfigVar(key="price_band_refresh_time",
                  ...
                  type_str="float",
                  default=86400,
                  required_if=lambda: trinity_config_map.get("moving_price_band_enabled").value,
    ...
    "order_level_amount":
        ConfigVar(key="order_level_amount",
                  ...
                  required_if=lambda: trinity_config_map.get("order_levels").value > 1,
                  type_str="decimal",
    ...
    "ask_order_optimization_depth":
        ConfigVar(key="ask_order_optimization_depth",
                  ...
                  required_if=lambda: trinity_config_map.get("order_optimization_enabled").value,
                  type_str="decimal",
    ...
    "price_type":
        ConfigVar(key="price_type",
                  ...
                  type_str="str",
                  required_if=lambda: trinity_config_map.get("price_source").value != "custom_api",
                  default="mid_price",
    ...
    "price_source_derivative":
        ConfigVar(key="price_source_derivative",
                  ...
                  required_if=lambda: trinity_config_map.get("price_source").value == "external_market",
                  type_str="str",
